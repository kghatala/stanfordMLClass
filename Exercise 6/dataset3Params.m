...
error_mat = zeros(length(possibleC),length(possibleSigma));
for i = 1:length(possibleC)
  for j = 1:length(possibleSigma)
    ...
    error_mat(i,j) = predError;
  end
end

minError = min(min(error_mat));
[rowInd,colInd] = find(error_mat==minError);
C = possibleC(rowInd);
sigma = possibleSigma(colInd);
